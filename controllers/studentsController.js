...
exports.addStudent = async (req, res) => {
  try {
    ...
    if (!parent) {
      return res
        .status(400)
        .json({ message: "Parent with the given id does not exist" });
    }
    ...
    if (student) {
      return res
        ...
        .json({
          message:
            "Admission number has already been assigned to another student",
        });
    }
    // step 3
    ...
    if (!classroom) {
      return res
        .status(400)
        .json({ message: "Classroom with the given id does not exist" });
    }
    ...
    if (!classroom.students.includes(savedStudent._id)) {
      classroom.students.push(savedStudent._id), await classroom.save();
    }
...
exports.updateSudentDetails = async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({
        message: "Error updating the details of students",
        error: err.message,
      });
  }
};
